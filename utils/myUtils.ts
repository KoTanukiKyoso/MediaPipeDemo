...
export const postApi = async (url: string, arr: Dictionary, overlay: boolean = true, login: boolean = true,
    ...
    let {firebaseUserData, addMessage, emailVerified} = useStore();
    let tasks: any = useTasks();
    let response: ClientResponse = getClientResponse();
    if ((!firebaseUserData || !firebaseUserData.value || !firebaseUserData.value.token) && login) {
        addMessage({
    ...
    const r: Response = await fetch(`/api/${url}`, {
        ...
        headers: {
            ...
            'Authorization': `token ${firebaseUserData?.value?.token}`,
        },
    });
...
export const getClassFields = (obj: object): string[] => {
