...
import {useFirebaseAuth, useTasks} from "~/composables";

let tasks = useTasks();
const {isSignedIn, isSignedUp, addMessage, onAuthStateChanged, emailVerified} = useStore();
...
const signup = () => {
  ...
  if (process.env.NODE_ENV === "production" && email.value.indexOf("+") >= 0) {
    addMessage({
...
const sendVerify = (user: User | null = null) => {
  if (!user) {
    const auth = getAuth();
...
<template>
  <my-section-card white>
    <template v-if="showTitle" #title>
      <h2 v-if="[0,1].includes(pageType)">Sign up / in</h2>
      <h2 v-else-if="pageType===2">Sign up</h2>
    </template>
    <template v-if="showTitle" #subtitle>
      <span v-if="[0,1].includes(pageType)">登録/サインイン</span>
      <span v-else-if="pageType===2">新規登録</span>
    </template>

    <div v-if="isSignedUp">
      <h3 class="text-gray">新規登録完了！<br>確認メールを送信しています、メール確認後、リロードしてお試しください。</h3>
    </div>
    <div v-else-if="pageType===0" class="pt-4">
